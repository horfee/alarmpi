...
#ifdef RPI
    wiringPiISR(this->nReceiverPin, INT_EDGE_BOTH, &handleInterrupt);
#endif
  }
}
...
unsigned long RCSwitch::waitForValue() {

	std::unique_lock<std::mutex> lock(mutex);

	condition.wait(lock);
	if ( nReceiverPin == -1 ) return -1;
	long res = this->getReceivedValue();
	this->resetAvailable();
	return res;
}
/**
...
void RCSwitch::handleInterrupt() {
  ...
  if (duration > 5000 && duration > RCSwitch::timings[0] - 200 && duration < RCSwitch::timings[0] + 200) {    
    ...
    if (repeatCount == 2) {
		if (receiveProtocol1(changeCount) || receiveProtocol2(changeCount) ){
			// success
			condition.notify_all();
		}
		repeatCount = 0;
